...
wetness_c <- raster("wetness_cost_monforte.tiff")
wetness_cost <- projectRaster(wetness_c, elevation)
...
```{r, message=FALSE, warning=FALSE, paged.print=FALSE}
# GLM creation
covariates <- stack(elevation, tot_inso, aspect, slope, visual_prom, tpi100, wetness_cost, geol_cost, hydro_cost, lcp_cost, wind, tpi500)
names(covariates) <- c("elevation", "tot_inso", "aspect", "slope", "visual_prom", "tpi100", "wetness_cost", "geol_cost","hydro_cost", "lcp_cost", "wind", "tpi500")
locvals <- data.frame(extract(covariates, locations))
locvals$site <- locations$CID==1

mod <- glm(site~elevation+tot_inso+aspect+slope+visual_prom+tpi100+wetness_cost+geol_cost+hydro_cost+lcp_cost+wind+tpi500, data=locvals, family=binomial(logit))
summary(mod)
...
summary(mod2) 
logodds <- 7.2294176+(elevation* -0.0167014)+(pendiente_grados*-0.3456405)+(prominencia_visual*0.0458370)+(coste_branhas*-0.0017355)+(coste_hidrologia*0.0007585)
par(mfrow=c(1,1))
plot(logodds)
...
slope <- raster("grids/pend_grad-monf.tif")
wetness_cost <- raster("grids/coste_branhas_monforte")
hydro_cost <- raster("grids/coste_hidrologia_monforte")
visual_prom <- raster("grids/prominencia_visual_monforte.tif")
```
